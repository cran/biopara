pboot <- function(bioparatarget,bioparasource,bioparanruns, data2, statistic, R, sim = "ordinary", stype = "i", strata = rep(1, n), L = NULL, m = 0, weights = NULL, ran.gen = function(d,p) d, mle = NULL,...)
{
...
#
	...
	j <- max(c(1:n)[L>lim])
	if (i==j)
		stop(paste("Likelihood exceeds",lim,"at only one point"))
	if (i==1) bot <- -Inf
	else {	i <- i+c(-1,0,1)
		x <- theta[i]
		y <- L[i]-lim;
...
isMatrix <- function(x) length(dim(x)) == 2
	...
	pbootfullfunctioncall<-match.call();
	pbootfullfunctioncall<-capture.output(pbootfullfunctioncall);
	pboottemp<-textConnection("pbootfxncall", "w");
	cat(pbootfullfunctioncall, file=pboottemp);
	close(pboottemp);
	pbootfullfunctioncall<-pbootfxncall;
	bioparatempvar <- nchar(pbootfullfunctioncall);	
	for(bioparaiterator in 1:bioparatempvar)
	{
		if(bioparaiterator+4>bioparatempvar)
		{
			break;
		}
		if(substr(pbootfullfunctioncall,bioparaiterator,bioparaiterator+4) == "data2")
		{
			pbootfullfunctioncall<-substr(pbootfullfunctioncall,bioparaiterator+5,bioparatempvar)
			break;
		}
	}
        results<-biopara(bioparatarget,bioparasource,1,list("setenv"))
        results<-biopara(bioparatarget,bioparasource,list("boot","data", "statistic", "R", "sim", "stype","strata", "L","m", "weights", "ran.gen", "mle"),list("setenv"))
 	results<-biopara(bioparatarget,bioparasource,bioparanruns,list(paste("bioparaout<-boot(data",pbootfullfunctioncall,";bioparaout[[8]]<-toString(bioparaout[[8]]);bioparaout")));	
      	return(results);	
}
